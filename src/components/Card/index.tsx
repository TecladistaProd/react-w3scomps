...
class Card extends PureComponent<Props, State> {
  ...
  render() {
    ...
    if(this.props.borderType && !(this.props.borderType.match(/(normal|rounded)/g)))
      throw new Error('borderType should be "normal" or "rounded"')
    return(
      <div className={this.props.borderType === 'rounded' ? 'rounded' : ''} style={this.props.style} css={styles}>
        {
          this.props.children
        }
      </div>
